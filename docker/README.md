...
```sh
docker run -v $PWD:/work ghcr.io/digline/digline:0.21.0 compare --suite eval/suite.py
```
...
```sh
digline() { docker run --rm -v "$PWD:/work" ghcr.io/digline/digline:0.21.0 "$@"; }
...
| Tag | |
|---|---|
| `0.21.0` | that version of digline, and the plugin versions it shipped with |
| `0.21` | the newest patch on that minor |
...
```sh
docker run --user "$(id -u):$(id -g)" -v "$PWD:/work" ghcr.io/digline/digline:0.21.0 \
...
```dockerfile
FROM ghcr.io/digline/digline:0.21.0
