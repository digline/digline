...
```sh
docker run -v $PWD:/work ghcr.io/digline/digline:0.5.0 compare --suite eval/suite.py
```
...
```sh
digline() { docker run --rm -v "$PWD:/work" ghcr.io/digline/digline:0.5.0 "$@"; }
...
```sh
docker run --user "$(id -u):$(id -g)" -v "$PWD:/work" ghcr.io/digline/digline:0.5.0 \
...
```dockerfile
FROM ghcr.io/digline/digline:0.5.0
