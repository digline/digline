...
```sh
docker run -v $PWD:/work ghcr.io/digline/digline:0.13.2 compare --suite eval/suite.py
```
...
```sh
digline() { docker run --rm -v "$PWD:/work" ghcr.io/digline/digline:0.13.2 "$@"; }
...
| Tag | |
|---|---|
| `0.13.2` | that version of digline, and the plugin versions it shipped with |
...
```sh
docker run --user "$(id -u):$(id -g)" -v "$PWD:/work" ghcr.io/digline/digline:0.13.2 \
...
```dockerfile
FROM ghcr.io/digline/digline:0.13.2
