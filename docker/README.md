...
```sh
docker run -v $PWD:/work ghcr.io/digline/digline:0.12.1 compare --suite eval/suite.py
```
...
`compare`, `diff`, `promote`, `register`, `report`, `explain`, `log`, `rejudge`,
`list`, `migrate`. The exit code is the answer: `0` fine, `1` got worse, `2` could not
be judged.
...
```sh
digline() { docker run --rm -v "$PWD:/work" ghcr.io/digline/digline:0.12.1 "$@"; }
...
| Tag | |
|---|---|
| `0.12.1` | that version of digline, and the plugin versions it shipped with |
| `0.12` | the newest patch on that minor |
...
```sh
docker run --user "$(id -u):$(id -g)" -v "$PWD:/work" ghcr.io/digline/digline:0.12.1 \
...
```dockerfile
FROM ghcr.io/digline/digline:0.12.1
