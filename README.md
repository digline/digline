...
| `digline log` | which model answered, read down the stored runs: what each side sent, what the provider said answered, the rolls between them, and every run that recorded nothing named for what it is. A roll is declared by the record, never deduced from scores. Never a gate: it exits 0 whatever it finds — [ADR 0020](docs/adr/0020-the-reading-across-runs.md) |
...
`0.12.1`, pre-1.0. The offline cycle — write the suite, run, promote, compare,
